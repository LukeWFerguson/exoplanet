...
# train[train.columns.difference(['rowid', 'kepid', 'kepoi_name', 'kepler_name', 'koi_disposition', 'koi_pdisposition', 'koi_tce_delivname'])]
model.fit(train[['dec']], train['koi_disposition_num'])
total = time.time() - start_time
print('Model trained in ' + str(round(total, 2)) + ' seconds.')

print('Predicting test dataset...')
start_time = time.time()
# test['prediction'] = model.predict(test[test.columns.difference(['rowid', 'kepid', 'kepoi_name', 'kepler_name', 'koi_disposition', 'koi_pdisposition', 'koi_tce_delivname', 'koi_disposition'])])
test['prediction'] = model.predict(test[['dec']])
total = time.time() - start_time
